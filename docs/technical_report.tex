...
\paragraph{Stale hedge weights.}
Hedge weights are computed at position entry and held fixed until exit.
As rates move, the PCA exposures of the signal bond change while the hedge
stays constant, generating drift in factor neutrality. Periodic hedge
re-solving (e.g., weekly) is the correct fix and would likely improve both
the rising-rate and steep-curve regime results.
